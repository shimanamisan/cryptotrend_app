...
class GetCoinsTweet extends Command
{
        ...
        for ($i = 0; $i < count($search_key); $i++) {
            ...
            for ($k = 0; $k < self::SEARCH_REQUEST_LIMIT; $k++) {
                try {
                    ...
                    // オブジェクト形式で返ってくる
                    $response_result = $connection->get(
                        "search/tweets",
                        $params
                    );
                    ...
                    \Log::debug("============ メッセージ終了 ============");
                    \Log::debug(
                        "リクエスト結果が空なのでオブジェクトを配列に変換する処理をスキップします。"
                    );
                    \Log::debug("   ");
                ...
                if ($connection->getLastHttpCode() !== 200) {
                    ...
                    // 15分間待機して処理を継続する
                    \Log::debug(
                        "サーバ側でエラー若しくはAPI制限に掛かりました。処理を待機します"
                    );
                    \Log::debug("900秒待機中・・・・");
    ...
    private function twitterOauth2()
    {
        ...
        // アプリ認証用のベアラートークンを取得
        $_bearer_token = $connection->oauth2("oauth2/token", [
    ...
    private function getSearchDate($date)
    {
        switch ($date) {
            case "hour":
                return $before_hour =
                    date("Y-m-d_H:i:s", strtotime("-1 hour", time())) . "_JST"; //カウント開始の時間

            case "day":
                return $before_hour =
                    date("Y-m-d_H:i:s", strtotime("-1 day", time())) . "_JST"; //カウント開始の時間

            case "week":
                return $before_hour =
                    date("Y-m-d_H:i:s", strtotime("-1 week", time())) . "_JST"; //カウント開始の時間
