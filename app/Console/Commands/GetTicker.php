    ...
    public function handle(Coin $coin)
    {
        ...
        *****************************************/
        ...
        foreach ($currency_pairs as $currency_kye => $currency) {
            ...
            \Log::debug('取得したJSON形式の情報を配列に変換しています。：' . print_r($jsonArray, true));
            \Log::debug('    ');
            ...
            \Log::debug('各銘柄の過去24時間の最高取引価格を取り出しています。' . $max_price);
            \Log::debug('    ');
            \Log::debug('各銘柄の過去24時間の最安取引価格を取り出しています。' . $low_price);
            \Log::debug('    ');
            try {
        ...
        *****************************************/
        ...
        // 文字列から変換
        $jsonArray = json_decode($json, true);
        \Log::debug('コインチェックAPIから取得したJSON形式の情報を配列に変換しています。：' . print_r($jsonArray, true));
        \Log::debug('    ' . self::BCT);
        ...
        *****************************************/
        ...
        foreach ($pair_list as $pair) {
            ...
            // 文字列から変換
            $jsonArray = json_decode($json, true);
            \Log::debug('ビットバンクAPIから取得したJSON形式の情報を配列に変換しています。：' . print_r($jsonArray, true));
            \Log::debug('    ');
