...
class TwitterAuthController extends Controller
{
    ...
    public function getTwitterCallback()
    {
        try {
            ...
            \Log::debug(
                "ログイン中のアカウントに、Twitterアカウントを新規登録します"
            );
            \Log::debug("   ");
            ...
            if (Auth::check()) {
                ...
                if ($userInfo === false) {
                    ...
                    \Log::debug("   ");
                    return redirect("/twuserlist")->with(
                        "error_message",
                        "既に認証されているアカウントです。他のTwitterアカウントを選択してください。"
                    );
                }
                ...
                if ($userInfo !== null) {
                    ...
                    session([
                        "access_token_secret" =>
                            $userInfo->twitter_token_secret,
                    ]);
                    \Log::debug(
                        "新規登録に成功しました。セッションを格納し、画面遷移します"
                    );
                    \Log::debug("   ");
                    ...
                    if (empty($twitterUserList)) {
                        ...
                        $follow_list = $this->fetchFollowTarget(
                            $twuser->id,
                            $connection
                        );
                        ...
                        if (!empty($follow_list)) {
                            \Log::debug(
                                "自分のフォローしているユーザーを取得しています"
                            );
                            \Log::debug("   ");

                            // 配列を比較し共通しているものを出力する
                            $follow_list_intersect = array_intersect(
                                $twitterUserList,
                                $follow_list
                            );
                            ...
                            if (!empty($follow_list_intersect)) {
                                foreach (
                                    $follow_list_intersect
                                    as $follow_user_id
                                ) {
                                    \Log::debug(
                                        "DBと重複してるフォロー済みのユーザーをfollowsテーブルに登録しています  TwitterID：" .
                                            $follow_user_id .
                                            "  UserID：" .
                                            $userInfo->id
                                    );
                ...
                return redirect("/login")->with(
                    "error_message",
                    "ログイン有効期限が切れました。再度ログインしてからTwitterアカウント認証を行ってください。"
                );
            }
        } catch (\Exception $e) {
            \Log::debug(
                "ログインに失敗しました。例外の処理に入っています。" .
                    $e->getMessage()
            );
            ...
            return redirect("/login")->with(
                "error_message",
                "Twitterアカウント認証に失敗しました。しばらく経過してから、再度ログインして実行してください。"
            );
        }
    }
