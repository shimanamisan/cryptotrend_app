    ...
    *********************************************************/
    public function follow(Request $request)
    {
        ...
        */
        ...
        $system_limit_time = $SystemManager->one_day_system_follow_limit_time;
        ...
        // 時刻でフォロー制限をかける際の時刻をユーザーから取得（リクエスト制限をかけた際に、現在の時刻より15分後の時刻が入っている）
        $follow_limit_time = Auth::user()->follow_limit_time;
        // フォローするユーザーのIDを格納
        ...
        Log::debug('現在の時刻です：'.$now_time . '  制限の時間です：'.$follow_limit_time);
        Log::debug('   ');
        ...
        if ($person_follow_limit_count < self::DAY_PERSON_LIMIT &&
            ...
            // 現在の時刻がusersテーブルに保存されている時間より進んでいれば処理を行う
            // follow_limit_time には、自動フォロー機能でフォローした際に 15フォロー/15分 に引っかかっていれば、
            // その時の時刻より15分後の時刻が入っている
            if ($now_time > $follow_limit_time) {
                ...
                Log::debug('15フォロー/15分の制限中です。');
                Log::debug('    ');
                return response()->json(['error' => 'フォロー制限中です。しばらくお待ち下さい。'], 403);
            }
        } else {
